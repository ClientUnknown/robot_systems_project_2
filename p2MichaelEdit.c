...
task main() {
	...
	repeatUntil(nClockMinutes >= 1.5) {
		...
		while (getUSDistance(ultrasonic) > 5 || !isStuck()) {
			...
			if (atBlackLine() || getColorName(S4) == ENEMY_BASE_COLOR) {
				turnAround();
				forward(50);
				sleep(500);
			}
...
void turnRight() {
	...
	repeatUntil(getGyroDegrees(gyro) >= 90 || getGyroDegrees(gyro) >= -90) {
...
bool atBlackLine() {
	return (colorBlack == getColorName(S4));	// replace S1 with color sensor **
}

bool inBase() {
	return (BASE_COLOR == getColorName(S4));	// replace S1 with color sensor **
}
